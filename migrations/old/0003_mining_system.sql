...
CREATE TABLE IF NOT EXISTS mining_packages (
  ...
  name TEXT NOT NULL,
  hash_rate REAL NOT NULL, -- TH/s
  price REAL NOT NULL, -- USD
  daily_earnings REAL NOT NULL, -- Estimated USD per day
  duration_days INTEGER NOT NULL, -- Package duration
  description TEXT,
...
CREATE TABLE IF NOT EXISTS user_miners (
  ...
  daily_rate REAL NOT NULL, -- USD per day
...
-- Mining Sessions Table (Track start/stop events)
CREATE TABLE IF NOT EXISTS mining_sessions (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  user_id INTEGER NOT NULL,
  miner_id INTEGER NOT NULL,
  action TEXT NOT NULL, -- start, stop, pause
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (user_id) REFERENCES users(id),
  FOREIGN KEY (miner_id) REFERENCES user_miners(id)
);
...
INSERT INTO mining_packages (name, hash_rate, price, daily_earnings, duration_days, description) VALUES
